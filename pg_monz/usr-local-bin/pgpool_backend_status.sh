...
case "$APP_NAME" in
        pgpool.nodes)
                ...
                sending_data=$(for backendrecord in $pool_nodes; do
                                                        ...
                                                        BACKENDWEIGHT=`echo $backendrecord | awk -F, '{print $6}'`
                                                        BACKENDROLE=`echo $backendrecord | awk -F, '{print $7}'`
                                                        BACKENDREPLICATIONDELAY=`echo $backendrecord | awk -F, '{print $11}'`
                                                        BACKENDLASTSTATUSCHANGE=`echo $backendrecord | awk -F, '{print $14}'`
